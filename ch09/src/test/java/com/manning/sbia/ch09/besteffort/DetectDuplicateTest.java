...
public class DetectDuplicateTest {
    ...
    @Test
    public void simulateDuplicate() throws Exception {
        JobParameters jobParameters = new JobParametersBuilder().addLong("date", System.currentTimeMillis()).toJobParameters();
        ...
        when(redeliveredNotProcessed.getJMSRedelivered()).thenReturn(true);
        when(redeliveredNotProcessed.getObject()).thenReturn(createOrder("3", createItem("3")));

        when(redeliveredProcessed.getJMSRedelivered()).thenReturn(true);
        when(redeliveredProcessed.getObject()).thenReturn(createOrder("14", createItem("14")));

        when(mockReader.read()).thenReturn(redeliveredProcessed).thenReturn(redeliveredNotProcessed).thenReturn(null);
    ...
    private void fillInQueue() {
        jmsTemplate.convertAndSend(createOrder("1", createItem("1"), createItem("2", (short) 2)));
        jmsTemplate.convertAndSend(createOrder("2", createItem("1", (short) 2)));
        jmsTemplate.convertAndSend(createOrder("3", createItem("3")));
        jmsTemplate.convertAndSend(createOrder("4", createItem("4")));
        jmsTemplate.convertAndSend(createOrder("5", createItem("5")));
        jmsTemplate.convertAndSend(createOrder("6", createItem("6")));
        jmsTemplate.convertAndSend(createOrder("7", createItem("7")));
        jmsTemplate.convertAndSend(createOrder("8", createItem("8")));
        jmsTemplate.convertAndSend(createOrder("9", createItem("9")));
        jmsTemplate.convertAndSend(createOrder("10", createItem("10")));
        jmsTemplate.convertAndSend(createOrder("11", createItem("11")));
        jmsTemplate.convertAndSend(createOrder("12", createItem("12")));
        jmsTemplate.convertAndSend(createOrder("13", createItem("13")));
    }
